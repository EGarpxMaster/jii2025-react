...
// Registro: incluye BRAZALETE 1..500 (único). Se permite string numérico desde el frontend.
const RegistroSchema = z.object({
  ...
  programa: z
    ...
    .optional(),
  brazalete: z.preprocess((val) => {
    if (typeof val === "string") {
      const n = Number(val.trim());
      return Number.isFinite(n) ? n : val;
    }
    return val;
  }, z.number().int().min(1, "Brazalete debe ser entre 1 y 500").max(500, "Brazalete debe ser entre 1 y 500")),
});
...
const EquipoSchema = z.object({
  ...
  emailsMiembros: z
    .array(z.string().email("Correo de miembro inválido"))
...
// NUEVO: Verificar si un brazalete ya existe
const checkBrazaleteExists = (brazalete: number): Promise<boolean> => {
  return new Promise((resolve, reject) => {
    const query = "SELECT COUNT(*) AS count FROM participantes WHERE brazalete = ?";
    db.query(query, [brazalete], (err, results: any[]) => {
...
const validateParticipantsForTeam = async (
  emails: string[]
): Promise<{ valid: boolean; errors: string[]; participantes: any[] }> => {
  const errors: string[] = [];
...
async function generarConstanciaPDF(participante: any, asistencias: any[]): Promise<Buffer> {
  try {
    ...
    const nombreCompleto = `${participante.primerNombre} ${participante.segundoNombre || ""} ${participante.apellidoPaterno} ${participante.apellidoMaterno}`
      .trim()
      .toUpperCase();
...
// Verificación y utilidades de registro:
//  - action=check-email&email=...
//  - action=check-brazalete&brazalete=...
app.get("/api/registro", async (req, res) => {
  const { action, email, brazalete } = req.query;
  ...
  if (action === "check-brazalete" && typeof brazalete === "string") {
    try {
      const n = Number(brazalete.trim());
      if (!Number.isFinite(n) || n < 1 || n > 500) {
        // fuera de rango: marcamos explicitamente
        return res.json({ unique: false, error: "out_of_range" });
      }
      const exists = await checkBrazaleteExists(n);
      return res.json({ unique: !exists });
    } catch (error) {
      console.error("Error checking brazalete:", error);
      return res.status(500).json({ error: "Error verificando brazalete" });
    }
  }
...
// Registrar participante (incluye brazalete)
app.post("/api/registro", async (req, res) => {
  try {
    ...
    if (emailExists) {
      return res.status(409).json({ error: "El correo ya fue registrado", field: "email" });
    }

    // Validar unicidad de brazalete
    const brazaleteExists = await checkBrazaleteExists(brazalete);
    if (brazaleteExists) {
      return res.status(409).json({ error: "El brazalete ya fue registrado", field: "brazalete" });
    }

    const insertQuery = `
      INSERT INTO participantes 
      (apellido_paterno, apellido_materno, primer_nombre, segundo_nombre, email, telefono, categoria, programa, brazalete)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    db.query(
      ...
      (insertErr, insertResults) => {
        if (insertErr) {
          // Manejo explícito de duplicados por índice único
          if ((insertErr as any).code === "ER_DUP_ENTRY") {
            const msg = String((insertErr as any).message || "");
            if (msg.includes("email")) {
              return res.status(409).json({ error: "El correo ya fue registrado", field: "email" });
            }
            if (msg.includes("brazalete")) {
              return res.status(409).json({ error: "El brazalete ya fue registrado", field: "brazalete" });
            }
          }
...
// Obtener participante por email (incluye brazalete)
...
// Verificar posibilidad de constancia
app.get("/api/constancia/verificar", async (req, res) => {
  ...
  try {
    ...
    const asistencias = await getAsistenciasWithConferencias(email as string);
...
app.get("/api/constancia/generar", async (req, res) => {
  ...
  try {
    const participante = await getParticipantByEmail(email as string);
    if (!participante) {
      return res.status(404).json({ error: "Participante no encontrado" });
    }
...
app.get("/api/equipos/by-participant", async (req, res) => {
  ...
  try {
    const equipo = await getEquipoByParticipantEmail(email as string);
