...
// MODIFICADO: Se usa fechaInicio y fechaFin
type Conferencia = {
 id: number;
 titulo: string;
 ponente?: string | null;
...
const AsistenciaPage: React.FC = () => {
 ...
 // Ordenar conferencias: 1. Disponibles, 2. Próximas, 3. Pasadas
 const sortedConferencias = useMemo(() => {
    ...
    const getStatus = (conf: Conferencia) => {
        ...
        if (now >= inicio && now <= fin) return 1; // Disponible ahora
        if (now < inicio) return 2; // Próxima
        return 3; // Pasada
    };
    ...
    return [...conferencias].sort((a, b) => {
        ...
        // Si el estado es el mismo, ordenar por fecha de inicio
    ...
    {participante && conferencias.length > 0 && (
     <div>
      ...
      <div style={{ display: "grid", gap: "1rem" }}>
       {/* MODIFICADO: Se itera sobre el array ordenado */}
        ...
        return (
         <div
          ...
          style={{
           ...
           // MODIFICADO: Estilo visual para conferencias no disponibles
           opacity: !isAvailable && !yaRegistrado ? 0.6 : 1,
          }}
         >
          {yaRegistrado && (
           <div style={{
          ...
          <div style={{ 
           ...
           {/* MODIFICADO: Se usa fechaInicio */}
           📅 {new Date(conf.fechaInicio).toLocaleString("es-MX", {
            weekday: "long",
            year: "numeric",
            month: "long", 
            day: "numeric",
          ...
          {yaRegistrado ? (
           ...
           <button
            ...
            // MODIFICADO: El botón se deshabilita si está cargando o no está disponible
            ...
            style={{
             ...
             fontWeight: "600",
             color: "white",
             background: !isAvailable ? "#A0AEC0" : (cargando ? "#ccc" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)"),
             border: "none",
             borderRadius: 8,
             cursor: cargando || !isAvailable ? "not-allowed" : "pointer",
             transition: "all 0.3s ease",
             transform: cargando ? "none" : "translateY(0)",
            }}
            onMouseEnter={(e) => {
             if (!cargando && isAvailable) {
           ...
           >
            {/* MODIFICADO: Texto del botón cambia según el estado */}
